...
class Transaction{};(function(){const template=document.createElement("template");template.innerHTML=`
    ...
    <slot></slot>
    `;class ActionsContainer extends HTMLElement{constructor(){super();this.attachShadow({mode:"open"});this.shadowRoot.appendChild(template.content.cloneNode(!0))}}
customElements.define("actions-container",ActionsContainer)})();(function(){const template=document.createElement("template");template.innerHTML=`
    <style>
    :host {
        align-items: center;
...
registerActions(){const inputs=this.editor.querySelectorAll("form input");this.actions.forEach((action)=>{const type=parseInt(action.dataset.action);action.addEventListener("click",()=>{this.operation=Operation.Create;this.transactionType=type;clearForm(type);let typeInput=this.editor.querySelector("form input[name=TransactionType]");typeInput.value=type;this.editor.show();this.changeTheme("#999999")})});function clearForm(type){inputs.forEach(input=>{input.value=input.type!="number"?"":"0.00"})}}
...
loadBalances(){const data={accountId:this.account.Id};const balances=this.card.querySelectorAll("card-balance");axios.get("report/account_balance.php",{params:data}).then(response=>{const payload=response.data;const content=payload.content;if(!payload.state){alert(`Oops! ${content}`);return}
...
return;input.value=name!=""?data[name]:Math.abs(data.Amount)});this.changeTheme("#999999");this.editor.show()}
save(){let endpoint=`transaction/crud/${Operation[this.operation]}.php`;console.log(Operation[this.operation]);let form=this.editor.querySelector("form");let data=new FormData(form);data.append("AccountId",this.account.Id.toString());let trans=Object.fromEntries(data.entries());axios.post(endpoint,trans).then(response=>{if(response.data.state)
this.refresh();this.operation=Operation.Create;this.editor.hide();this.changeTheme("#dadada")}).catch(error=>{console.log(error)})}
delete(trans){this.handlePost("transaction/crud/delete.php",trans)}
post(trans){this.handlePost("transaction/post/post.php",trans)}
cancel(trans){this.handlePost("transaction/post/cancel.php",trans)}
