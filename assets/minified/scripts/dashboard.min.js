...
customElements.define("transaction-panel",TransactionPanel)})();const CREATE=0;const UPDATE=1;class TransactionManager{constructor(config){for(var key in config){this[key]=config[key]}
this.account;this.operation=CREATE;this.registerMenu();this.registerEditor()}
registerMenu(){this.contextMenu=globalContext.addMenu("transactions_row",this.container);var options=["Update","Post","Delete","Cancel"];var menuOptions=options.reduce((mo,o)=>{const option=new ContextMenuOption(o);option.visible(trans=>trans.Posted==(o=="Cancel"));mo.push(option);return mo},[]);menuOptions[0].onClick(trans=>this.updateBtnClicked(trans));menuOptions[1].onClick(trans=>this.postBtnClicked(trans.Id));menuOptions[2].onClick(trans=>this.deleteBtnClicked(trans.Id));menuOptions[3].onClick(trans=>this.cancelBtnClicked(trans.Id));this.contextMenu.addOptions(...menuOptions)}
...
this.container.innerHTML=null;let transactions=this.groupTransactions(content);for(var key in transactions){const group=this.newGroup(key,transactions[key]);this.container.appendChild(group)}}).catch(error=>{alert("An error occured.");console.log(error)})}
groupTransactions(trans){var groups={};for(var t of trans){const key=t.Date+t.Posted;(groups[key]??=[]).push(t)}
return groups}
newGroup(date,trans){var group=$("<div>").addClass("transaction-group");var header=$("<p>").addClass("transaction-group-header");var body=$("<div>").addClass("transaction-group-body");var dateTime=DateTime.parse(date.slice(0,-1));header.text(dateTime.toString("MMM. dd, yyyy"));const status=trans[0].Posted?"actual":"projection";header.addClass(status);group.append(header);group.append(body);for(var t of trans){let row=this.newRow(t);body.append(row)}
return group[0]}
