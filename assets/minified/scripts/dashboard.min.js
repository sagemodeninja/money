...
onContext(e){let target;let isDocument=!1;let contextId;let dataIndex;do{if(target){target=target?.parentNode}else{target=e.target}
isDocument=target===document;contextId=!isDocument?target.getAttribute("data-context-id"):null}
while(!isDocument&&!contextId);target=syn.wrap(target);dataIndex=target.data("index");if(contextId){e.preventDefault();this.reset();this.activeTrigger=target;this.triggerTop=e.clientY;this.triggerLeft=e.clientX;this.show(contextId,dataIndex);target.addClass("active")}}
...
reset(){this.element.empty();this.element.removeClass("active");this.element.attr("style","");this.activeTrigger?.removeClass("active");this.activeTrigger=undefined;this.triggerTop=undefined;this.triggerLeft=undefined}}
...
customElements.define("transaction-panel",TransactionPanel)})();const CREATE=0;const UPDATE=1;class TransactionManager{constructor(config){this.card=config.card;this.container=config.container;this.editor=config.editor;this.command=config.command;this.account;this.operation=CREATE;this.registerMenu();this.registerEditor()}
registerMenu(){this.contextMenu=globalContext.addMenu("transactions_row",this.container);let updateOption=new ContextMenuOption("Update");let postOption=new ContextMenuOption("Post");let deleteOption=new ContextMenuOption("Delete");let cancelOption=new ContextMenuOption("Cancel");updateOption.visible(isProjection);postOption.visible(isProjection);deleteOption.visible(isProjection);cancelOption.visible(trans=>trans.Posted);function isProjection(trans){return!trans.Posted};updateOption.onClick(trans=>this.updateBtnClicked(trans));postOption.onClick(trans=>this.postBtnClicked(trans.Id));deleteOption.onClick(d=>this.deleteBtnClicked(trans.Id));cancelOption.onClick(trans=>this.cancelBtnClicked(trans.Id));this.contextMenu.addOptions(updateOption,postOption,deleteOption,cancelOption)}
registerEditor(){let editor=$("#editor");this.command.addEventListener("click",()=>{this.operation=CREATE;editor.find("input").each((idx,ipt)=>{$(ipt).val("")});this.editor.show();this.changeTheme("#999999")});const dissmissEditorBtn=document.querySelector("#dismiss_editor_dialog_btn");dissmissEditorBtn.addEventListener("click",()=>{this.changeTheme("#dadada");this.editor.hide()});$("#save_btn").click(e=>{let url=this.operation==CREATE?"transaction/crud/create.php":"transaction/crud/update.php";$.ajax({url:url,method:"POST",data:editor.serialize()+`&AccountId=${this.account.Id}`,dataType:"JSON",success:payload=>{if(payload.state)
this.refresh();this.operation=CREATE;this.editor.hide();this.changeTheme("#dadada")}})})}
load(account){this.account=account;this.refresh()}
...
loadBalances(){const data={AccountId:this.account.Id};const balances=this.card.querySelectorAll("card-balance");axios.get("report/balances.php",{params:data}).then(response=>{const payload=response.data;const content=payload.content;if(!payload.state){alert(`Oops! ${content}`);return}
balances[0].innerText=toCurrency(content.Running);balances[1].innerText=toCurrency(content.Projected)}).catch(error=>{alert("An error occured.");console.log(error)})}
loadTransactions(){const data={AccountId:this.account.Id};axios.get("transaction/crud/read.php",{params:data}).then(response=>{const payload=response.data;const content=payload.content;if(!payload.state){this.container.innerHTML=`<p class="centered">Oops! ${content}</p>`;return}
this.container.innerHTML=null;let transactions=this.groupTransactions(content);Object.keys(transactions).forEach(key=>{let group=this.newGroup(key,transactions[key]);this.container.appendChild(group)})}).catch(error=>{alert("An error occured.");console.log(error)})}
groupTransactions(trans){var groups=trans.reduce((g,t)=>{const key=`${t.Date}_${t.Posted}`;g[key]??=[];g[key].push(t);return g},{});var sorted=Object.keys(groups).sort(()=>-1).reduce((o,k)=>{o[k]=groups[k].sort(()=>-1);return o},{});return sorted}
newGroup(date,trans){var group=$("<div>").addClass("transaction-group");var header=$("<p>").addClass("transaction-group-header");var body=$("<div>").addClass("transaction-group-body");var dateTime=DateTime.parse(date.slice(0,-2));header.text(dateTime.toString("MMM. dd, yyyy"));if(!trans[0].Posted){const status=trans.Posted?"actual":"projection";header.addClass(status)}
group.append(header);group.append(body);$.each(trans,(i,t)=>{let row=this.newRow(t);body.append(row)});return group[0]}
newRow(trans){const status=trans.Posted?"actual":"projection";var row=$(`<div class="transaction-row ${status}">`);var main=$("<div>").addClass("main-content");var desc=$(`<div class='transaction-description'><p>${trans.Description}</p></div>`);var summary=$("<div class='transaction-summary'>");main.append(desc);main.append(summary);row.append(main);var isDebit=trans.Debit>trans.Credit;var transAmount=isDebit?trans.Debit:trans.Credit;transAmount=toCurrency(transAmount.toString());var amount=$(`<p>${!isDebit ? "-" : ""} PHP ${transAmount}</p>`);var ref=$("<p>REF: N/A</p>");summary.append(amount);summary.append(ref);row[0].addContext(this.contextMenu,trans);const actions=$("<div>").addClass("actions-container");row.append(actions);if(status==="projection"){const editAction=this.newAction("edit","Edit");const postAction=this.newAction("post","CompletedSolid");const deleteAction=this.newAction("delete","Delete");editAction.click(()=>{collapseActions();this.updateBtnClicked(trans)});postAction.click(()=>{collapseActions();this.postBtnClicked(trans.Id)});deleteAction.click(()=>{collapseActions();this.deleteBtnClicked(trans.Id)});actions.append(editAction);actions.append(postAction);actions.append(deleteAction)}else{const cancelAction=this.newAction("delete","Cancel");actions.append(cancelAction);cancelAction.click(()=>{collapseActions();this.cancelBtnClicked(trans.Id)})}
...
const delta=touch.pageX-initialTouch.pageX;const left=Math.min(0,initialLeft+delta);if(Math.abs(delta)>0)
...
updateBtnClicked(data){this.operation=UPDATE;let editor=$("#editor");editor.find("input").each((idx,ipt)=>{ipt=$(ipt);let name=ipt.attr("name");ipt.val(data[name])});this.changeTheme("#999999");this.editor.show()}
deleteBtnClicked(id){$.ajax({url:"transaction/crud/delete.php",method:"POST",data:{Id:id},dataType:"JSON",success:payload=>{if(payload.state)
this.refresh();}})}
postBtnClicked(id){$.ajax({url:"transaction/post/post.php",method:"POST",data:{Id:id},dataType:"JSON",success:payload=>{if(payload.state)
this.refresh();}})}
cancelBtnClicked(id){$.ajax({url:"transaction/post/cancel.php",method:"POST",data:{Id:id},dataType:"JSON",success:payload=>{if(payload.state)
this.refresh();}})}
changeTheme(theme){document.querySelector('meta[name="theme-color"]').setAttribute("content",theme)}};var body;var refreshCommand;var toggleCommand;var closedAccounts;var contextMenu;var transactionPanel;var transaction;$(document).ready(()=>{var isShowClosed=!1;body=document.querySelector("#layout_body");refreshCommand=document.querySelector("#refresh_command");toggleCommand=document.querySelector("#toggle_closed_command");transactionPanel=document.querySelector("transaction_panel");transaction=new TransactionManager({card:document.querySelector("#transaction_card"),container:document.querySelector(".transaction-container"),editor:document.querySelector("#editor_dialog"),command:document.querySelector("#create_command")});closedAccounts=[];contextMenu=globalContext.addMenu("accounts_card",body);refreshCommand?.addEventListener("click",refreshAccounts);toggleCommand?.addEventListener("click",()=>{toggleCommand.icon=isShowClosed?"View":"Hide";toggleCommand.label=isShowClosed?"Show Closed Accounts":"Hide Closed Accounts";closedAccounts.forEach(card=>{if(isShowClosed)
...
arr.push({Id:categoryId,Title:acct.Category,Color:acct.CategoryColor});return arr},[]);let categorized=content.reduce((map,acct)=>map.set(acct.CategoryId,[...map.get(acct.CategoryId)??[],acct]),new Map());refreshBalances(categories,categorized)}else{body.innerHTML=`<p class="centered">Oops! ${accounts.content}</p>`}}).catch(error=>{console.dir(error)})}
function refreshBalances(categories,categorized){categorized.forEach((accounts,_category)=>{let category=categories.find(cat=>cat.Id==_category)??{};let color=category?.Color??"9E9E9E";let title=category?.Title??"Uncategorized";const container=$('<div class="category"></div>');const accountsGrid=$('<div class="accounts-grid"></div>');const tag=$(`<fluent-symbol-icon symbol="Tag" foreground="#${color}" font-size="13" class="tag"></fluent-symbol-icon>`);const label=$(`<span class="title">${title}</span>`);body.appendChild(container[0]);body.appendChild(accountsGrid[0]);container.append(tag);container.append(label);$.each(accounts,(idx,account)=>{let card=newCard(account,title);accountsGrid.append(card)})})}
function newCard(account,category){let accountNumber=account.AccountNumber?.slice(-4)??"••••";let bankIcon=account.BankIcon;const card=$("<account-card>");const runningBalance=$("<card-balance>...</card-balance>");const projectedBalance=$("<card-balance>...</card-balance>");const accountBankIcon=$('<img class="account-bank-icon" slot="icon">');card.append(runningBalance);card.append(projectedBalance);card.append(accountBankIcon);card.prop("title",account.Title);card.prop("number",accountNumber);card.prop("category",category);runningBalance.prop("title","Actual");projectedBalance.prop("title","Projection");if(bankIcon!=null){accountBankIcon.attr("src",`assets/images/bank_icons/${bankIcon}.svg`);if(bankIcon=="master_card")
...
$.ajax({url:"report/account_balance.php?accountId="+account.Id,method:"GET",dataType:"JSON",success:payload=>{if(payload.state){let balances=payload.content;runningBalance.text(toCurrency(balances.Balance));projectedBalance.text(toCurrency(balances.Projection))}else{runningBalance.text("!");projectedBalance.text("!");console.error(payload.content)}}});card.click(()=>{transaction_panel.show();transaction.load(account)});return card}
