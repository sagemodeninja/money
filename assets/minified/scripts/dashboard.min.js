...
save(){const operation=Operation[this.operation].toLowerCase();const endpoint=`transaction/crud/${operation}.php`;let form=this.editor.querySelector("form");let data=new FormData(form);data.append("AccountId",this.account.Id.toString());let trans=Object.fromEntries(data.entries());axios.post(endpoint,trans).then(response=>{if(response.data.state)
